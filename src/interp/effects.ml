...
module Make (State : Utils.Types.T) (Env : Env.S) (Err : sig
  type t
  ...
  *)
  ...
  let read : (Env.t, 'e) t =
    { run = fun ~reject:_ ~accept state step env -> accept env state step }
  ...
  *)
  ...
  let[@inline always][@specialise] fail (env : Err.t) : 'a m =
    { run = fun ~reject ~accept:_ state step _ -> reject env state step }

  let fail_map (f : State.t -> Err.t * State.t) : 'a m = 
    { run = fun ~reject ~accept:_ state step _ -> let e, s = f state in reject e s step }
  ...
  *)
  ...
  let run (x : 'a m) (init_state : State.t) (init_env : Env.t) : ('a, Err.t) result * State.t * Step.t =
    x.run init_state Step.zero init_env
      ~reject:(fun e state step -> Error e, state, step) 
      ~accept:(fun a state step -> Ok a, state, step)

  let run_safe (x : 'a s) (init_state : State.t) (init_env : Env.t) : 'a * State.t * Step.t =
    x.run init_state Step.zero init_env
      ~reject:absurd
